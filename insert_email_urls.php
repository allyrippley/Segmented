...
if (isset($_POST['save'])) {
    ...
    if (strlen($html) > 0) {
        foreach($html->find('a') as $element)  {
            if ((strpos($element->href, 'mailto:') === false) && (strpos($element->href, $baseTrackingUrl[0]) === false)) {
                if (strpos($element->href, '://') === false) { $element->href = $baseTrackingUrl[0] . $element->href; }
    ...
    while ($row = mysql_fetch_array($result)) {
                ...
                if ($row['source']=="email") {
                    $emailContent = str_ireplace($row['destination_url'], $row['generated_url'] . "&uid=$" . "USER_ID", $emailContent);
                }
	}
