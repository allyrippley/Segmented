...
if ($links_table_result) {
    while ($row = mysql_fetch_array($links_table_result)) {
		if (is_null($row{'menu_position'})) {
			$links_data[$i]['destination_url'] = htmlspecialchars_decode($row{'destination_url'});
...
if (isset($_SESSION["article_title"])) {
    $pageTitle = htmlspecialchars($_SESSION["article_title"]);
...
if (isset($_SESSION["article_icon"])) {
    $pageIcon = $_SESSION["article_icon"];
    unset($_SESSION["article_icon"]);
}

if (isset($_SESSION["article_header_image"])) {
    $pageHeaderImage = $_SESSION["article_header_image"];
    unset($_SESSION["article_header_image"]);
}
...
if (isset($_SESSION["edit_flag"]) && !isset($_SESSION["article_flag"])) {
    ...
    if ($result) {
        ...
        while ($row = mysql_fetch_array($result)) {
            ...
            $_SESSION["article_title_" . ($j + 1)] = htmlspecialchars($row{'title'});
			$_SESSION["article_summary_" . ($j + 1)] = $row{'summary'};
			$_SESSION["article_footer_" . ($j + 1)] = $row{'footer'};
                        $_SESSION["article_header_image_" . ($j + 1)] = $row{'header_image'};
                        $_SESSION["article_icon_" . ($j + 1)] = $row{'icon_image'};
...
if (isset($_SESSION["article_title_" . $articlePageId[1]])) {
    ...
    $pageTitle = htmlspecialchars(stripslashes($pageTitle));
...
if (isset($_SESSION["article_header_image_" . $articlePageId[1]])) {
    $pageHeaderImage = $_SESSION["article_header_image_" . $articlePageId[1]];
    $pageHeaderImage = mysql_real_escape_string(stripslashes($pageHeaderImage));
	//unset($_SESSION["article_summary_" . $articlePageId[1]]);
}

if (isset($_SESSION["article_icon_" . $articlePageId[1]])) {
    $pageIcon = $_SESSION["article_icon_" . $articlePageId[1]];
    $pageIcon = mysql_real_escape_string(stripslashes($pageIcon));
	//unset($_SESSION["article_summary_" . $articlePageId[1]]);
}
                        ...
                        <div class="content-table">
                            ...
                            <div class="content-table-b">
								...
								<div><ul>
									...
									<li><input type="text" name="article_title" id="article_title" value="<?php echo htmlspecialchars_decode(stripslashes($pageTitle)); ?>"><br /><br /></li>
								</ul></div>
								<div><ul>
									<li><div style="font-size:14px;">Article Summary</div></li>
									<li><input type="text" name="article_summary" id="article_summary" value="<?php echo htmlspecialchars($pageSummary, ENT_QUOTES); ?>"><br /><br /></li>
								</ul></div>
                                                                <div><ul>
									<li><div style="font-size:14px;">Article Header Image</div></li>
									<li><input type="text" name="article_header_image" id="article_header_image" value="<?php echo htmlspecialchars($pageHeaderImage, ENT_QUOTES); ?>"><br /><br /></li>
								</ul></div>
                                                                <div><ul>
									<li><div style="font-size:14px;">Article Icon Image</div></li>
									<li><input type="text" name="article_icon" id="article_icon" value="<?php echo htmlspecialchars($pageIcon, ENT_QUOTES); ?>"><br /><br /></li>
								</ul></div>
